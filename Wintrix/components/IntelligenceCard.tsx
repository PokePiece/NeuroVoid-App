...
const IntelligenceCard: React.FC<IntelligenceCardProps> = ({ post, handleTagClick, handleEdit, handleDelete }) => {
  ...
  const handleCopy = async () => {
    try {
      ...
      // Fallback for environments where clipboard API might not be available (e.g., some iframes)
      setCopied('failed'); // Indicate that copying failed
    } finally {
  ...
  return (
    <div className="prompt_card">
      ...
      {post.description || post.content_type && (
        <p className="my-2 font-inter text-sm text-gray-300">{post.description || post.content_type}</p>
      )}
