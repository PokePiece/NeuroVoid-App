...
export default function DemoComputer({ texture, isPlaying, onClickScreen, onVideoEnded, ...props }) {
  const group = useRef();
  const { nodes, materials } = useGLTF('/models/computer.glb');

  const txt = useVideoTexture(texture ? texture : 'textures/project/myproject1.mp4');
  ...
  // useEffect to handle initial video state and 'ended' event listener
  useEffect(() => {
    if (txt) {
      txt.flipY = false;
      const videoElement = txt.image;

      // Ensure the video is paused and reset when the texture (and thus the video) changes
      videoElement.pause();
      videoElement.currentTime = 0; // IMPORTANT: Reset current time here for new videos

      const handleVideoEnded = () => {
        if (onVideoEnded) {
          onVideoEnded(); // Notify parent that video has finished
        }
        videoElement.currentTime = 0; // Reset to start for next play after natural end
      };
  ...
  // useEffect to control video playback based on the 'isPlaying' prop
  useEffect(() => {
    if (txt && txt.image) {
      const videoElement = txt.image;
      if (isPlaying) {
        ...
        videoElement.pause();
        // We don't reset currentTime here, allowing it to resume from pause point
      }
    }
  }, [isPlaying, txt]);
